...
int main()
{
	...
	while(1){
		...
		printf("Please input N(0 is exit):");// Input N
		scanf("%d", &N);
		
		if (!N)	return 0;
		
		while ((N+1)%2!=0 || (N+1)%3!=0 || (N+1)%5!=0){
			printf("input again:");// Input N
			scanf("%d", &N);
		}
